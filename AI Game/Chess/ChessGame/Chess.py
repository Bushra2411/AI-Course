...
import pygame      # Used for handling graphics and user interaction
import chess       # a Python library that manages chess rules, board state, and legal moves.
...
def highlight_squares(screen, board, selected_square):    # Function to highlight selected square and possible moves.
    if selected_square is None:
        return
    s = selected_square
    col = chess.square_file(s)  
    row = 7 - chess.square_rank(s)    # used to flip the rank so that the chessboard aligns with PyGame’s top-down screen coordinates.

    # Highlight selected square (yellow border)
    pygame.draw.rect(screen, (255, 255, 0), pygame.Rect(col * SQ_SIZE, row * SQ_SIZE, SQ_SIZE, SQ_SIZE), 5)   #  Draw yellow border on the selected square.
...
def draw_pieces(screen, board):   # Function to draw all pieces based on current board state.
    for square in chess.SQUARES:  #Loop through all 64 squares (0 to 63).
...
def evaluate(board):
    ...
    for piece in board.piece_map().values():
        value = values.get(piece.piece_type, 0)     # Get the value of the current piece (default 0 if unknown).
...
def minimax(board, depth, maximizing):
    ...
    if maximizing:
        ...
        for move in board.legal_moves:
            board.push(move)
            eval, _ = minimax(board, depth-1, False)   #all minimax assuming opponent's turn next.
            board.pop()
            if eval > max_eval:
                max_eval = eval
                best_move = move
